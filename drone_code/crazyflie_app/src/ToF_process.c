...
uint16_t DIS_REACT = 1500; // mm //2200
uint16_t DIS_SLOW = 1200; // mm //1500 for me 1000
uint16_t DIS_STOP = 800; // mm //1000 for me 700
uint16_t DIS_FEAR = 300; // mm //200
...
FlyCommand_t Process_ToF_Image(VL53L5CX_ResultsData* p_tof_results)
{
    ...
    for (int i= 0; i<ROW; ++i)
        for (int j= 0; j<COL; ++j)
        {
            ...
            {
                ToF_distances_matrix[i][j] = UINT16_MAX; //init as invalid
